...
global $wpdb;
$contact_info = $wpdb->get_results('SELECT wum.user_id,wu.user_email,wu.user_login FROM wp_users wu, wp_usermeta wum WHERE wu.ID = wum.user_id AND wum.meta_key = "contact_dealer_id" AND wum.meta_value =' . $dealer_id);
...
if ($action == 'view') {
    ...
    foreach ($dealer_array as $key => $value) {
        ...
        $actions['delete'] = sprintf(
            '<a href="%1$s" id="pmsafe_dealers_delete" data-id="' . $value['dealer_id'] . '">%2$s</a>',
